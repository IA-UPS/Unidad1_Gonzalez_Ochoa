...
funRidge <- function(x) train(clase ~ ., data = x, method = 'glmnet', trControl = trControl, tuneGrid = expand.grid(alpha = 0, lambda = seq(0, 1, by = 0.001)), trace= F)

ridge.lista <- lapply(datos.train.lista, funRidge)
...
funLasso <- function(x) train(clase ~ ., data = x, method = 'glmnet', trControl = trControl, tuneGrid = expand.grid(alpha = 1, lambda = seq(0, 1, by = 0.001)), trace= F)

lasso.lista <- lapply(datos.train.lista, funLasso)

lasso.pred <- vector("list", length = length(datos.lista))

for(l in 1:length(datos.lista)){
  lasso.pred[[l]] <- predict(ridge.lista[[l]], datos.test.lista[[l]])
}
